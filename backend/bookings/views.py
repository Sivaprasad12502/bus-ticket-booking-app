...
from .serializers import BusSerializer,RouteSerializer,TripSerializer,SeatSerializer,PassengerSerializer,PayementSerializer,BookingSerializer
...
def create_bookings(request):
    if request.method=='GET':
        ...
        seats=Seat.objects.filter(trip=trip,seat_number__in=seat_numbers,is_booked=False)
        ...
        booking=Booking.objects.create(user=user,trip=trip,total_amount=total_amount)
        booking.seats.set(seats)
        #Mark seats as booked
        seats.update(is_booked=True)
        return Response({"booking_id":booking.id,"total_amount":total_amount,"booked_seats":seat_numbers},status=status.HTTP_201_CREATED)
