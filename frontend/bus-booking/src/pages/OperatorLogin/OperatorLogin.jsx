...
const OperatorLogin = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (formData) =>
      await axios.post(`${apiUrl}users/operator/login/`, formData),
    onSuccess: ({ data }) => {
      console.log("Operator login successful:", data);
      setOperatorAccessToken(data.access);
      setOperatorRefreshToken(data.refresh);
      const userData = {
    ...
    onError: (error) => {
      ...
      toast.error(
        error.response?.data?.error ||
          "Operator Login failed. Please try again."
      );
    },
  });
  const { data } = mutation;
  if (data) {
    console.log("operator login data", data);
  }
      ...
      <div className="card">
        <div className="left">
          <div className="left__content">
            ...
            <p>Manage your bus operations, routes,</p>
          </div>
        </div>
        <div className="right">
